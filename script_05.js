...
function formatName(){
  ...
  if(validateString(nome, "alert")) {
    ...
    const formattedArray = arrayName.map((word)=> {
      if(word.toLowerCase() === 'de' || word.toLowerCase() === 'do' || word.toLowerCase() === 'da') return word.toLowerCase()
        const palavraTratada = word.toLowerCase().replace(word[0], word[0].toUpperCase())
